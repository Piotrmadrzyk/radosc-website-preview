# -*- coding: utf-8 -*-
"""Scala cząstkowe noty w tools/kolekcja-jaroslaw/noty.json i robi kontrolę GLOBALNĄ.
...
PC = os.path.join(WORK, 'jar_pc.json')
OUT = os.path.join(ROOT, 'tools', 'kolekcja-jaroslaw', 'noty.json')
FIXES = os.path.join(ROOT, 'tools', 'kolekcja-jaroslaw', 'noty_fixes.json')
...
def load():
    ...
    notes = {}
    import glob
    parts = sorted(glob.glob(os.path.join(SRC, '*.json')))
    print('plików cząstkowych:', len(parts))
    for p in parts:
...
def main():
    ...
    for n in notes:
        ...
        if 'Jarosław' in t:
            errs.append('%s: „Jarosław” w tytule — %r' % (s, t))
    ...
    # --- obiekty, które nie są pocztówkami, nie mogą być tak nazwane ---
    # (dwanaście odbitek fotograficznych + rysunek PT:6:13; „papier pocztówkowy”
    #  i „rewers pocztówkowy” to opis podłoża, nie nazwa obiektu — te przechodzą)
    pcrec = {r['sygnatura']: r for r in json.load(open(PC))}
    for n in notes:
        sg = n['sygnatura']
        typ = str(pcrec.get(sg, {}).get('typ') or '')
        if typ.startswith(('fotografia', 'rysunek')):
            body = n['nota'] + ' ' + n['sig_typ']
            for m in re.finditer(r'pocztówk\w*', body, re.I):
                ctx = body[max(0, m.start() - 30):m.end() + 14]
                if re.search(r'(papier\w*|rewers\w*|nadruk\w*|format\w*)\s+pocztówkow', ctx, re.I):
                    continue
                errs.append('%s (%s): nazwane „%s” — …%s…' % (sg, typ.split()[0], m.group(0), ctx))
